...
This module is intentionally independent of any model vendor.  A reasoned minister
run must cross ``required_horus_call`` before final judgment.  The minister states
what information is needed and what would count as adequate evidence.  Horus keeps
source-selection independence, gathers or fails to gather, and returns the exact
sources searched, used, rejected, and still missing.

The boundary enforces the constitutional seam discovered in the August 2026 runs:
the judge may direct attention but may not silently curate its own ground.
"""
...
class HorusExchangeError(ValueError):
    """Raised when a minister-Horus exchange violates the hard contract."""
...
def _nonempty_strings(values: Any, field: str) -> list[str]:
    ...
    _require(all(isinstance(v, str) and v.strip() for v in values),
             f"{field} must contain only non-empty strings")
    return values


def validate_query(query: Dict[str, Any]) -> Dict[str, Any]:
    """Validate the mandatory investigative request without choosing sources."""
    _require(isinstance(query, dict), "query must be an object")
    _require(query.get("record_type") == QUERY_RECORD_TYPE,
             f"record_type must be {QUERY_RECORD_TYPE}")
    for field in ("query_id", "inquiry_id", "minister_id", "reason_for_request"):
        _require(isinstance(query.get(field), str) and query[field].strip(),
                 f"{field} is required")
    ...
    _require(isinstance(requirements, list) and requirements,
             "source_requirements must be a non-empty list")
    for index, requirement in enumerate(requirements):
        _require(isinstance(requirement, dict),
                 f"source_requirements[{index}] must be an object")
        for field in ("requirement", "rationale"):
            _require(isinstance(requirement.get(field), str) and requirement[field].strip(),
    ...
    _require(rule == SOURCE_SELECTION_RULE,
             "minister may specify evidentiary requirements but may not take source-selection authority from Horus")
    ...
    _require(isinstance(provenance.get("produced_by"), str) and provenance["produced_by"].strip(),
             "provenance.produced_by is required")
    ...
    _require(isinstance(commit, str) and len(commit) == 40 and all(c in "0123456789abcdef" for c in commit),
             "provenance.repository_commit must be a 40-character lowercase git SHA")
    return query


def validate_response(query: Dict[str, Any], response: Dict[str, Any]) -> Dict[str, Any]:
    """Validate Horus's answer and its complete source trail against the request."""
    ...
    _require(response.get("query_id") == query["query_id"],
             "Horus response query_id does not match the minister request")
    _require(response.get("requesting_minister") == query["minister_id"],
             "Horus response requesting_minister does not match the request")
    _require(response.get("status") in RESPONSE_STATUSES,
             "Horus response status is invalid")
    _require(response.get("completeness") == "PENDING_PROBE",
             "Horus may not self-certify completeness")
    ...
    _require(used_refs <= searched_refs,
             "every source used by Horus must also be recorded in sources_searched")
    _require(rejected_refs <= searched_refs,
             "every rejected source must also be recorded in sources_searched")
    ...
    if response["status"] == "NOT_GATHERED":
    ...
    if response["status"] == "PARTIALLY_GATHERED":
        _require(bool(returned) and bool(unfilled),
                 "PARTIALLY_GATHERED must contain both returned and unfilled records")

    provenance = response.get("provenance")
    _require(isinstance(provenance, dict), "response provenance is required")
    hcommit = provenance.get("horus_repository_commit", "")
    _require(isinstance(hcommit, str) and len(hcommit) == 40 and all(c in "0123456789abcdef" for c in hcommit),
             "provenance.horus_repository_commit must be a 40-character lowercase git SHA")
    _require(isinstance(provenance.get("generated_at"), str) and provenance["generated_at"].strip(),
             "provenance.generated_at is required")
    return response


def required_horus_call(
    query: Dict[str, Any],
    horus_gather: Callable[[Dict[str, Any]], Dict[str, Any]],
) -> Dict[str, Any]:
    """Hard call seam that every reasoned minister run must cross.

    ``horus_gather`` is the Horus-side implementation.  It may search any source
    that satisfies the request and must disclose the complete source trail.  The
    minister cannot substitute a local answer for this call.
    """
    validate_query(query)
...
def exchange_digest(query: Dict[str, Any], response: Dict[str, Any]) -> str:
    """Stable digest for binding a report/run manifest to the exact exchange."""
    validate_response(query, response)
    payload = json.dumps(
        {"query": query, "response": response}, sort_keys=True, separators=(",", ":")
    ).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def write_exchange(root: Path, query: Dict[str, Any], response: Dict[str, Any]) -> Dict[str, str]:
    """Persist the request and response as separate, attributable records."""
    validate_response(query, response)
    ...
    return {
        ...
        "sha256": exchange_digest(query, response),
    }
