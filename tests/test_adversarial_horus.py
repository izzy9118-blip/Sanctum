...
class AdversarialHorusCall(unittest.TestCase):
    def test_provisional_requires_disconfirmation_need(self):
        item = provisional()
        del item["propositions"][0]["disconfirmation_need"]
    ...
    def test_adversarial_source_use_must_be_disclosed_as_searched(self):
        ...
        response = adversarial_response(query)
    ...
    def test_not_gathered_keeps_disconfirmation_gap_visible(self):
        query = build_adversarial_query(provisional())
        response = adversarial_response(query)
        ...
        response["unfilled_requests"] = [
            {
                "information_need": query["information_needed"][0],
                "reason": "No qualifying contradictory or qualifying record was acquired.",
            }
        ]
