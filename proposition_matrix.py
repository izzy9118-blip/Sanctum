...
The President performs semantic alignment; code does not infer agreement. Code instead
requires exhaustive, exact, non-rewritten representation of every sovereign minister
proposition before Presidential synthesis may proceed.
"""
...
def build_inventory(packages: Dict[str, dict]) -> dict:
    ...
    for minister_id in sorted(packages):
        ...
        _require(package.get("record_type") == "final_judgment_package",
                 f"{minister_id} package is not a final_judgment_package")
        _require(package.get("minister_id") == minister_id,
                 f"{minister_id} package minister identity mismatch")
...
def validate_matrix(packages: Dict[str, dict], matrix: dict) -> dict:
    ...
    _require(matrix.get("record_type") == MATRIX_RECORD_TYPE,
             f"record_type must be {MATRIX_RECORD_TYPE}")
    _require(matrix.get("alignment_authority") == ALIGNMENT_AUTHORITY,
             "matrix alignment_authority must be PRESIDENTIAL_ALIGNMENT")
    _require(matrix.get("certification") == CERTIFICATION,
             "matrix may not self-certify correctness")
    ...
    _require(matrix.get("participating_ministers") == ministers,
             "matrix participating_ministers must exactly match the package set in sorted order")
    _require(matrix.get("package_bindings") == inventory["package_bindings"],
             "matrix package bindings do not match exact final judgment packages")

    source = {
        (p["minister_id"], p["proposition_id"]): p
        for p in inventory["propositions"]
    }
    seen_refs = set()
    ...
    for rindex, row in enumerate(rows):
        ...
        _require(isinstance(row_id, str) and row_id.startswith("ROW-"),
                 f"rows[{rindex}].matrix_row_id is invalid")
        _require(row_id not in row_ids, f"duplicate matrix_row_id: {row_id}")
        row_ids.add(row_id)
        question = row.get("comparison_question")
        _require(isinstance(question, str) and question.strip(),
                 f"rows[{rindex}].comparison_question is required")
        entries = row.get("minister_entries")
        _require(isinstance(entries, list) and len(entries) == len(ministers),
                 f"rows[{rindex}] must contain exactly one entry per participating minister")
        entry_ids = [e.get("minister_id") if isinstance(e, dict) else None for e in entries]
        _require(entry_ids == ministers,
                 f"rows[{rindex}] minister entries must appear exactly once in sorted minister order")

        for eindex, entry in enumerate(entries):
            state = entry.get("state")
            minister_id = entry["minister_id"]
            props = entry.get("propositions")
            ...
            for pindex, ref in enumerate(props):
                _require(isinstance(ref, dict), f"matrix proposition reference must be an object")
    ...
    expected = set(source)
    missing = expected - seen_refs
    extra = seen_refs - expected
    _require(not missing, f"matrix omits propositions: {sorted(missing)}")
    _require(not extra, f"matrix contains unknown propositions: {sorted(extra)}")
    return matrix


def required_presidential_matrix_call(
    packages: Dict[str, dict],
    president_align: Callable[[dict], dict],
) -> dict:
    """Hard call seam: synthesis may not occur before this validated alignment call."""
    inventory = build_inventory(packages)
...
def synthesis_payload(packages: Dict[str, dict], matrix: dict) -> dict:
    """Return the only conforming structured input for Presidential synthesis."""
    validate_matrix(packages, matrix)
    return {
        "record_type": "presidential_synthesis_input",
        "inquiry_id": matrix["inquiry_id"],
        "proposition_matrix": matrix,
        "minister_packages": packages,
        "rule": "SYNTHESIS_MUST_PRESERVE_MATRIX_DISAGREEMENT_AND_SILENCE",
    }
